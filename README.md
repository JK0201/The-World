...
## Outro
  ...
  <details>
    ...
    <ul>
      <li><b>버전 2.0 업데이트 (UI/UX 리펙터링) :</b>
      <br>
      React와 R3F(React Three Fiber)를 활용하여 SPA(Single Page Application)로 전환하면서 UI/UX를 근본적으로 개선할 예정입니다. 기존 쿼터뷰 방식에서 발생하는 사용자의 캐릭터 조작 번거로움과 개발 단계의 복잡성 및 디자인 피로도를 줄이는 것이 주요 목표입니다. 새로 도입될 스크롤 방식은 스크롤 높이에 따라 3D 모델이 변화하고, 특정 지점에서 관련 섹션이 활성화되도록 설계할 것입니다. 이는 사용자 경험을 향상시키며, 유지보수와 업데이트의 효율을 높일 것입니다.
      </li>
      <li><b>홈서버 하드웨어 업그레이드 :</b>
      <br>
      데이터 보호와 시스템 성능을 동시에 향상시키기 위해 추가적인 하드디스크가 확보되면, 현재 RAID1 구성에서 RAID10으로 업그레이드 할 예정입니다. RAID10은 RAID1의 데이터 복제(Mirroring) 기능과 RAID0의 데이터 분할(Striping)을 결합한 방식으로, 데이터 손실의 위험을 최소화하면서도 빠른 데이터 접근 속도를 제공할 것입니다.
      </li>
      <li><b>서버 보안 강화 및 자동화 개선</b>
      <br>
      현재 GitHub Actions를 통해 이미지를 Docker Hub로 빌드 및 배포하는 과정은 자동화되어 있습니다. 다음 단계로, 서버의 'docker-compose.yml' 실행도 자동화할 계획입니다. 이를 위해 SSH 키 기반 인증을 도입하고 Workflow 중에 서버에 안전하게 접속하여 'docker-compose up -d' 명령을 자동으로 실행할 수 있도록 구성할 예정입니다. 이러한 개선을 통해 전체 배포 프로세스의 자동화를 완성하고, 보안을 한층 더 강화할 예정입니다.
      </li>
    </ul>
  </details>
<br>
